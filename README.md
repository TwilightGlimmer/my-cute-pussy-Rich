...
不强制置顶，启动时不抢焦点，不显示任务栏入口；游戏和其他前台窗口可以覆盖小猫。右键小猫选择 **Close** 会停止动画和声音并退出进程。按键操作需要小猫窗口获得焦点。鼠标移动可触发不同方向的注视。

Windows 版在动作结束后安静等待 7–10 秒，再随机播放一个动作；每次选择的概率如下。眨眼保持头部和身体不动，仅更新眼睛区域。
...
观察和低头复用尺寸稳定的注视姿势；思考保持头部固定，使用现有动作的混合过渡，共 19 帧、约 2.7 秒。每次跳跃随机连续跳 2–3 次，自动触发和手动触发相同。
