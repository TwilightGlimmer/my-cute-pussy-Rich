...
窗口置顶，启动时不抢焦点，不显示任务栏入口。右键小猫选择 **Close** 会停止动画和声音并退出进程。按键操作需要小猫窗口获得焦点。鼠标移动可触发不同方向的注视。
...
所有动作使用 `cat-atlas.png` 对应行的完整原始帧和原始顺序，不拼接头部、不叠加混合帧、不用其他动作替换。低头保留原图集害羞、沮丧的表情；思考使用 6 个原始姿势，放慢到约 2.8 秒。原素材中的姿势、比例变化仍会保留。计时器每次最多前进一帧，卡顿时延长播放而不跳过姿势。每次跳跃随机完整连续跳 2–3 次，自动触发和手动触发相同。
